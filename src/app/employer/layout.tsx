...
export default function EmployerLayout({
  ...
  const titleMap: Record<string, string> = {
    ...
    '/employer/test/create': 'Online Test',
  };

  const currentTitle = titleMap[pathname] || 'Akij Resource';
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#f8f9fa] font-sans antialiased text-slate-900">
      {!isLoginPage && (
        <Header
          title={currentTitle}
          user={user?.role === 'employer' ? user : null}
          onLogout={handleLogout}
        />
      )}
      {/* For login page, header is slightly different in design, we will handle it in the page or keep it simple */}
      {isLoginPage && (
         <header className="flex h-16 items-center justify-center bg-white px-6 shrink-0 relative shadow-sm">
          <div className="absolute left-6 inset-y-0 flex items-center">
            <div className="flex flex-col items-start leading-none group text-[#2a2b7c]">
